...
describe('vuetify field props', () => {
  it('exposes vuetify-shaped props on field().props', () => {
    ...
    const properties = form.field('title').props
    expect(properties.modelValue).toBe('hi')
    expect(properties.label).toBe('Titre')
  ...
  it('exposes errors only once touched', async () => {
    ...
    const properties = form.field('title').props
    await nextTick()
    expect(properties.errorMessages).toEqual([])
    form.field('title').handleBlur()
    await nextTick()
    expect(properties.errorMessages).toEqual(['required'])
  })
