...
import type { AsuraSearchItem } from './asura.api'
import { mockNuxtImport } from '@nuxt/test-utils/runtime'
import { createPinia, setActivePinia } from 'pinia'
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { useToast } from '~/composables/toast.composable'
import { useAsuraSearchStore } from '../stores/asura-search.store'
import { useAsuraSearch } from './asura-search.composable'
...
describe('useAsuraSearch library', () => {
  it('adds a comic and stores the returned id', async () => {
    ...
    const asura = useAsuraSearch({ doSearch: false })
    useAsuraSearchStore().setComics([item('solo-leveling')])

    await asura.addComicInLibrary(item('solo-leveling'))

    expect(create).toHaveBeenCalledWith({ source: 'asurascans', slug: 'solo-leveling' })
    expect(useAsuraSearchStore().comics[0]?.internalId).toBe('c1')
  })

  it('toasts a dedicated message on a 409', async () => {
    create.mockResolvedValue(apiError(409))

    await useAsuraSearch({ doSearch: false }).addComicInLibrary(item('solo-leveling'))
  ...
  it('does not add a comic that already has an internal id', async () => {
    await useAsuraSearch({ doSearch: false }).addComicInLibrary(item('solo-leveling', 'c1'))
  ...
  it('removes a comic and clears its internal id', async () => {
    ...
    const asura = useAsuraSearch({ doSearch: false })
    useAsuraSearchStore().setComics([item('solo-leveling', 'c1')])

    await asura.removeComicFromLibrary(item('solo-leveling', 'c1'))

    expect(deleteById).toHaveBeenCalledWith('c1')
    expect(useAsuraSearchStore().comics[0]?.internalId).toBeUndefined()
  })

  it('does not delete a comic without an internal id', async () => {
    await useAsuraSearch({ doSearch: false }).removeComicFromLibrary(item('solo-leveling'))
  ...
  it('toasts an unknown error when delete fails', async () => {
    ...
    await useAsuraSearch({ doSearch: false }).removeComicFromLibrary(item('solo-leveling', 'c1'))
