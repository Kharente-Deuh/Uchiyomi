...
const { sort, chapters, loading, fetchChapters } = await vi.hoisted(async () => {
  ...
  return {
    ...
    chapters: vueRef<AsuraComicChapter[]>([]),
...
function chapter(number: number): AsuraComicChapter {
  return {
    id: `ch-${number}`,
...
async function mount(slug = 'solo-leveling'): Promise<VueWrapper> {
  return mountSuspended(
    ...
    },
    {
      global: {
        stubs: {
          AsuraComicChaptersItem: ItemStub,
