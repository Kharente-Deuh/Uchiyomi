...
vi.mock('~/features/asurascans/composables/asurascans-search.composable', () => ({
  useAsuraScansSearch: () => ({ removeComicFromLibrary }),
}))
...
async function mount(): Promise<{ wrapper: VueWrapper, comic: ReturnType<typeof ref<AsuraScansSearchItem | undefined>> }> {
  const show = ref(true)
  const comic = ref<AsuraScansSearchItem | undefined>(item())
  ...
  const wrapper = await mountSuspended(
    {
      render: () => h(VApp, () => [h(Delete, {
        ...
        'onUpdate:comic': (value: AsuraScansSearchItem | AsuraScansComicInfos | undefined) => {
          comic.value = value as AsuraScansSearchItem
        },
      })]),
    },
