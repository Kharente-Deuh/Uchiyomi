...
vi.mock('./asurascans.api', () => ({
  createAsuraScansApi: () => ({ getSeriesChapters }),
}))
...
function setup(): ReturnType<typeof useAsuraScansChapters> {
  const scope = effectScope()
  scopes.push(scope)
  const composable = scope.run(() => useAsuraScansChapters())
  if (!composable) {
    throw new Error('useAsuraScansChapters returned undefined')
  }
...
describe('useAsuraScansChapters polling', () => {
  ...
  it('polls every 2s without loading or toasts while a download is in progress', async () => {
    ...
    const asurascans = setup()
    await asurascans.fetchChapters()
    expect(asurascans.loading.value).toBe(false)

    await vi.advanceTimersByTimeAsync(2000)

    expect(getSeriesChapters).toHaveBeenCalledTimes(2)
    expect(asurascans.loading.value).toBe(false)
    expect(useAsuraScansChaptersStore().chapters[0]?.download).toBe(40)
  ...
  it('keeps the last list and does not toast when a poll fails', async () => {
    ...
    expect(useAsuraScansChaptersStore().chapters[0]?.download).toBe(10)
    expect(useToast().messages.value).toEqual([])
  })
  ...
  it('skips a tick while a fetch is already in flight', async () => {
    ...
    const poll = Promise.withResolvers<{ success: true, data: AsuraScansComicChapter[] }>()
...
describe('useAsuraScansChapters retryDownload', () => {
  it('refetches chapters without loading and starts polling after success', async () => {
    ...
    const asurascans = setup()
    await asurascans.fetchChapters()
    await asurascans.retryDownload('ch-1')

    expect(retryDownloadApi).toHaveBeenCalledWith('ch-1')
    expect(getSeriesChapters).toHaveBeenCalledTimes(2)
    expect(useAsuraScansChaptersStore().chapters[0]?.download).toBe(0)
    expect(asurascans.loading.value).toBe(false)
    expect(useToast().messages.value).toEqual([])
    ...
    expect(useAsuraScansChaptersStore().chapters[0]?.download).toBe(20)
  })
