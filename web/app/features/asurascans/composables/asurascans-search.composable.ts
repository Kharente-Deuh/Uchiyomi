...
import type { AsuraScansComicInfos, AsuraScansSearchItem, AsuraScansSort } from '../types'
...
export interface AsuraScansSearchComposable {
  ...
  removeComicFromLibrary: (comic: AsuraScansSearchItem) => Promise<boolean>
  addComicInLibrary: (comic: AsuraScansSearchItem | AsuraScansComicInfos) => Promise<string | undefined>
  addComicInLibraryLoading: Ref<Record<string, boolean>>
}

export function useAsuraScansSearch(opts: { doSearch: boolean }): AsuraScansSearchComposable {
  ...
  const sort = computed({
    get: () => store.sort,
    set: (value: AsuraScansSort) => store.setSort(value),
  })
  ...
  const comics = computed({
    get: () => store.comics,
    set: (value: AsuraScansSearchItem[]) => store.setComics(value),
  })

  const accumulatedComics = computed({
    get: () => store.accumulatedComics,
    set: (value: AsuraScansSearchItem[]) => store.setAccumulatedComics(value),
  })
  ...
  async function removeComicFromLibrary(comic: AsuraScansSearchItem): Promise<boolean> {
    if (!comic.internalId) {
      return false
    }
  ...
  async function addComicInLibrary(comic: AsuraScansSearchItem | AsuraScansComicInfos): Promise<string | undefined> {
