...
import MobileList from './index.vue'
import MobileItem from './Item.vue'
...
const sources: SourceDto[] = [makeSource('m1'), makeSource('m2', true), makeSource('m3')]

function wrap(
  props: Partial<InstanceType<typeof MobileList>['$props']> & { sources: SourceDto[] },
...
describe('extensionsSourceListMobile', () => {
  ...
  it('emits toggle with the source id when a child item emits toggle', async () => {
    ...
    const items = wrapper.findAllComponents(MobileItem)
    await items[0]!.find('.source-card-mobile').trigger('click')
    expect(emitted).toContain('m1')
  })
})
